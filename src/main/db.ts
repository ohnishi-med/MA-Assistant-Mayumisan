        ...
        db.serialize(() => {
            ...
            `, (err) => {
                ...
                db.serialize(() => {
                    ...
                    db.all("PRAGMA table_info(categories)", (err, catCols: any[]) => {
                        if (err) return;
                        ...
                        db.all("PRAGMA table_info(manuals)", (colsErr, manualCols: any[]) => {
                            if (colsErr) return;
        ...
        db.get('SELECT COUNT(*) as count FROM categories', (err, row: any) => {
            ...
            db.serialize(() => {
                ...
                rootCategories.forEach((cat, index) => {
                    ...
                    db.run('INSERT INTO categories (name, icon, level, display_order) VALUES (?, ?, 1, ?)', [cat.name, cat.icon, displayOrder], function (this: any, err) {
                        if (err) return;
